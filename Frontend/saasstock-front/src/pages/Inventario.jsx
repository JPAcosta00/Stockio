...
export default function Inventario() {
  ...
  const handleSaveSubmit = async (e) => {
    ...
    try {
      ...
      // Extraemos el mensaje del backend (puede venir como string o como propiedad .message / .title)
      const errorData = error.response?.data;
      let mensajeError = "Hubo un error al procesar la solicitud.";
      ...
      if (typeof errorData === 'string') {
        ...
        // Si C# devuelve errores de validación de ASP.NET Core (ValidationProblemDetails)
  ...
  return (
    <div className={`w-full max-w-full space-y-6 px-2 sm:px-4 lg:px-6 transition-colors duration-200 min-h-screen ${darkMode ? 'bg-zinc-950 text-zinc-100' : 'bg-slate-50 text-slate-800'}`}>
      ...
      <div className={`border p-5 rounded-2xl shadow-sm space-y-4 transition-colors ${darkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200'}`}>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
          ...
          <div className="w-full">
            <label className={`block text-[11px] uppercase tracking-wider mb-1.5 font-semibold ${darkMode ? 'text-zinc-400' : 'text-slate-500'}`}>
              Buscar por Nombre
            </label>
          ...
          <button
            ...
            className={`w-full font-semibold px-6 py-2.5 rounded-xl text-xs transition-colors border cursor-pointer shadow-sm ${
              darkMode 
        ...
        <div className={`flex flex-wrap items-center justify-between gap-3 pt-3 border-t ${darkMode ? 'border-zinc-800/80' : 'border-slate-100'}`}>
          ...
          {(filtroNombre || filtroPeriodo || filtroProveedor) && (
            <button
              onClick={() => {
                setFiltroNombre('');
                setFiltroPeriodo('');
      ...
      {totalPaginas > 1 && (
        <div className={`flex flex-col sm:flex-row items-center justify-between gap-4 border px-4 py-3 sm:px-6 rounded-2xl shadow-sm w-full ${darkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200'}`}>

          <div className={`text-xs ${darkMode ? 'text-zinc-400' : 'text-slate-600'} sm:hidden`}>
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className={`inline-flex items-center justify-center p-2 rounded-xl border text-xs font-medium disabled:opacity-40 transition-colors cursor-pointer ${
            ...
            {Array.from({ length: totalPaginas }, (_, i) => i + 1).map((numero) => {
              ...
              return (
                <button
                  ...
                  className={`w-8 h-8 rounded-xl text-xs font-semibold transition-colors cursor-pointer flex items-center justify-center border ${
                    esActual
      ...
      {modalImportarAbierto && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className={`w-full max-w-3xl rounded-2xl border p-6 shadow-2xl ${darkMode ? 'bg-zinc-900 border-zinc-800 text-white' : 'bg-white border-slate-200 text-slate-800'}`}>

            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold">Confirmar Importación Masiva</h3>
              <span className={`text-xs px-2.5 py-1 rounded-full ${darkMode ? 'bg-zinc-800 text-zinc-300' : 'bg-slate-100 text-slate-600'}`}>
            ...
            <div className={`flex items-center gap-3 p-3.5 rounded-xl border mb-4 ${darkMode ? 'bg-zinc-950 border-zinc-800' : 'bg-slate-50 border-slate-200'}`}>
              <input
                type="checkbox"
                id="chkActualizar"
                checked={actualizarExistentes}
                onChange={(e) => setActualizarExistentes(e.target.checked)}
                className="w-4 h-4 rounded border-zinc-700 text-[#5BA535] focus:ring-[#5BA535] cursor-pointer"
              />
              <label htmlFor="chkActualizar" className="text-xs font-medium cursor-pointer select-none">
                <strong>Actualizar productos existentes:</strong> Si el código de barras ya figura en el sistema, se sobrescribirán sus datos (precio, stock, etc.). Si se desmarca, solo se cargarán los productos nuevos.
              </label>
            </div>

            <div className={`max-h-60 overflow-y-auto rounded-xl border mb-6 ${darkMode ? 'border-zinc-800 bg-zinc-950' : 'border-slate-200 bg-slate-50'}`}>
            ...
            <div className="flex justify-end gap-3">
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-semibold border transition-colors cursor-pointer ${darkMode ? 'border-zinc-700 hover:bg-zinc-800 text-zinc-300' : 'border-slate-300 hover:bg-slate-100 text-slate-700'}`}
              >
                Cancelar
              </button>
              ...
              <button
                ...
                className="px-5 py-2 rounded-xl text-xs font-semibold bg-[#5BA535] hover:bg-[#4d8d2d] text-white transition-colors flex items-center gap-2 cursor-pointer"
              >
